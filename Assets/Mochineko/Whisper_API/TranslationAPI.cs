...
namespace Mochineko.Whisper_API
{
    ...
    /// </summary>
    public sealed class TranslationAPI
    {
        ...
        private readonly TranslationRequestBody requestBody;

        private static HttpClient HttpClient
            => HttpClientPool.PooledClient;

        /// <summary>
        /// Create an instance of Whisper translation API connection.
        /// </summary>
        ...
        public TranslationAPI(string apiKey, Model model = Model.Whisper1)
        {
            ...
            this.requestBody = new TranslationRequestBody(string.Empty, model);
        }
        ...
        /// <exception cref="ArgumentNullException">API Key must be set</exception>
        public TranslationAPI(string apiKey, TranslationRequestBody requestBody)
        {
            if (string.IsNullOrEmpty(apiKey))
            {
                throw new ArgumentNullException(nameof(apiKey));
        ...
        private static HttpRequestMessage CreateRequestMessage(
            ...
            TranslationRequestBody requestBody,
            Stream fileStream)
        {
        ...
        public async Task<string> TranslateFromFileAsync(string filePath, CancellationToken cancellationToken)
        {
            ...
            if (!TranslationRequestBody.IsAvailableFormat(filePath))
            {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!TranslationRequestBody.IsAvailableFormat(fileName))
            {
            ...
            {
                throw new Exception($"[Whisper_API.Translate] HttpResponseMessage is null.");
            }

            var responseText = await responseMessage.Content.ReadAsStringAsync();
            if (string.IsNullOrEmpty(responseText))
            {
                throw new Exception($"[Whisper_API.Translate] Response JSON is null or empty.");
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    throw new Exception(
                        $"[Whisper_API.Translate] It should not be be reached with status code:{responseMessage.StatusCode}.");
                }
            }
        }
    }
